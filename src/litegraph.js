    ...
    LGraphCanvas.prototype.processKey = function(e) {
        ...
        if (e.type == "keydown") {
            ...
            if ((e.key == "a" || e.key == "A") && e.shiftKey) {
                // This is a KeyboardEvent, so we don't have mouse coordinates
                // Luckily LGraphCanvas.prototype.processMouseMove stores
                //  mouse ClientXY in this.last_mouse_position array
                var clientXY = { clientX: this.last_mouse_position[0], clientY: this.last_mouse_position[1]};

                // The menu is consistently created too high, so move it down a bit
                // This might mess up scaling
                // Still haven't found a better way to do this
                clientXY.clientY += 20;

                // Create a fake event with ClientXY properties
                // Events other than MouseEvent and CustomEvent are ignored by ContextMenu
                // Creating an actual event object doesn't work because the target is not set
                var event = clientXY;
                event.constructor = MouseEvent;
                event.target = e.target;

                // Call method to show the AddNode menu with all categories
                // First 2 params are always ignored, last param is parent contextmenu
                // Since this is a root level menu, there is no parent
                LGraphCanvas.onMenuAdd(null, null, event, null);
            }
	...
	*/
    ...
    LGraphCanvas.onMenuAdd = function(node, options, e, prev_menu) {
        ...
        function inner_create(v, e) {
            var first_event = menu.getFirstEvent(); // Same functionality as prev_menu
